...
export class reviewprofitReport1 extends Component {
...
//  display: none;
    ...
    async load(){
        let model = "profit.redys";
        let domain=[]
        let fields = [];
        let filter=[]
        let method="process_data_report_profit"
     ...
     async updateData_after() {
      ...
      // async Prev(ev){
      //   let par_id=$(ev.currentTarget).data('par-id');
      //   if(par_id==this.state.par_id){
      //     this.state.accou_id = $(ev.currentTarget).data('acco-id');
      ...
      //   }
      //   else{
      //     await this._onClickClear_get(ev)
      //   }
      ...
      // }
    //   async _onClickClear_get(ev){
    //     this.state.par_id=$(ev.currentTarget).data('par-id');
    //     this.state.accou_id=$(ev.currentTarget).data('acco-id');
    //     console.log('_onClickClear_get')
    //     this.state.page=1
    //     this.state.Data=await this.getData(this.state.filter_list)
...
reviewprofitReport1.template = "yousentech_sale_reports.dy_report_re_profit_mrg_temp1";
reviewprofitReport1.events={
...
registry.category("actions").add("yousentech_sale_reports.profit_margin", reviewprofitReport1);
