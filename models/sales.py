...
import json
import os
class report_sales(models.TransientModel):
    _name='payment.redys'
    ...
    show_payment_this_company=fields.Boolean(
        string='Show sales for specific company',
        default=False,
    )
    ...
    def open_view(self):
        ...
        result={
            ...
            'tag': 'yousentech_sale_reports.payment_re',
        }
        return result
    ...
    def get_report_payment(self, *args, **kwargs):
        print("get_report_payment")
        data=[]

        sum_amount_untaxed=0
        sum_amount_with_tax=0
        sum_amount_out_tax=0
        sum_amount_tax=0
        sum_amount_total=0
        sum_quantity=0  
        show=False
                ...
                if arg['order_view']=='invoice':
                    ...
                    for recode in move_line:
                        ...
                        if move_line_details_analysis:
                            ...
                            sum_qut=sum(item.quantity for item in recode.invoice_line_ids.filtered(lambda x:x.move_id==recode.id))
                            amou_with_tax=sum(item.price_total for item in recode.invoice_line_ids.filtered(lambda x:x.price_total>x.price_subtotal))
                            amou_out_tax=sum(item.price_total for item in recode.invoice_line_ids.filtered(lambda x:x.price_total==x.price_subtotal))
                            amount_untaxed=recode.amount_untaxed
                            amount_tax=recode.amount_tax
                            amount_total=recode.amount_total
                            ...
                            sum_amount_with_tax+=round(amou_with_tax, 2)
                            sum_amount_out_tax+=round(amou_out_tax, 2)
                            sum_amount_tax+=round(amount_tax, 2)
                            sum_amount_total+=round(amount_total, 2)
                            ...
                            line={
                                ...
                                'amount_out_tax':round(amou_out_tax, 2),
                    ...
                    for recode in move_line:
                        ...
                        if arg['pro_cate_id']:
                            ...
                            # name_categ=self.env['product.category'].search(
                            #     [('id','in',[arg['pro_cate_id']])]
                            # )
                            # show_name_categ=name_categ.name
                        ...
                        for move_l in move_line_details:
                            amou_with_tax=sum(item.price_total for item in recode.invoice_line_ids.filtered(lambda x:x.price_total>x.price_subtotal and x.id==move_l.id))
                            amou_out_tax=sum(item.price_total for item in recode.invoice_line_ids.filtered(lambda x:x.price_total==x.price_subtotal and x.id==move_l.id))
                            price_subtotal=move_l.price_subtotal
                            amount_tax=move_l.price_total - move_l.price_subtotal
                            price_total=move_l.price_total
                            if recode.move_type=='out_refund':
                                amou_with_tax*=-1
                                amou_out_tax*=-1
                                price_subtotal*=-1
                                amount_tax*=-1
                                price_total*=-1
                            sum_amount_untaxed+=price_subtotal
                            sum_amount_with_tax+=round(amou_with_tax, 2)
                            sum_amount_out_tax+=round(amou_out_tax, 2)
                            sum_amount_tax+=round(amount_tax , 2)
                            sum_amount_total+=round(price_total, 2)
                            ...
                            line={
                                ...
                                'date':recode.date,
                                'partner_name':recode.partner_id.name,
                                'amount_untaxed':round(price_subtotal , 2)  ,
                                'amount_with_tax':round(amou_with_tax, 2),
                                'amount_out_tax':round(amou_out_tax, 2),
                                'amount_tax':round(amount_tax , 2) ,
                                'amount_total':round(price_total, 2),
        ...
        line_data={
            ...
            'sum_amount_out_tax':round(sum_amount_out_tax,2) ,
            'sum_amount_tax':round(sum_amount_tax,2) ,
            'sum_amount_total':round(sum_amount_total,2) ,
    ...
    def get_pdf_report(self):
        ...
        return self.env.ref('yousentech_sale_reports.print_sale_rpt').report_action(self, data=data)
    def get_excel_report(self):
        ...
        return self.env.ref('yousentech_sale_reports.excl_sale_rpt').report_action(self,data)  
class saleXlsx(models.AbstractModel):
    _name = 'report.yousentech_sale_reports.report_xlsx_sale_view'
    ...
    def generate_xlsx_report(self, workbook, data, patients):
        ...
        if data:
                ...
                if data['partner_id'] and data['partner_id']!=0 :
                    domain.append(('partner_id','in',[data['partner_id']]))
                    domain_analysis.append(('partner_id','in',[data['partner_id']]))
                if data['journal_id'] and data['journal_id']!=0:
                    domain.append(('journal_id','in',[data['journal_id']]))
                    domain_analysis.append(('journal_id','in',[data['journal_id']]))
                if data['salesman_id'] and data['salesman_id']!=0:
                    domain.append(('salesman_id','in',[data['salesman_id']]))
                     
                if data['from_date']!='YYYY-MM-DD'  and data['to_date']!='YYYY-MM-DD':
                    domain.append(('date', '>=',data['from_date']))
                    domain.append(('date', '<=',data['to_date']))
                    domain_analysis.append(('date', '>=',data['from_date']))
                    domain_analysis.append(('date', '<=',data['to_date']))
                   
                  
                user_id_now=self.env.user 
                if data['show_payment_this_company']:
                    
                    if data['company_ids'] :
                        # domain.append(('company_id','in',arg['company_ids']))
                        companies=self.env['res.company'].search([('id','in',data['company_ids'])])
                ...
                domain.append(('company_id','in',companies.ids))
                domain_analysis.append(('company_id','in',companies.ids))
                if data['hide_payment_cancel']==True:
                    domain.append(('state','!=','draft'))
                if data['type_payment']=='out_refund' :
                    domain.append(('move_type','=','out_refund'))
                elif data['type_payment']=='out_invoice' :
                    domain.append(('move_type','=','out_invoice'))
                else:
                    domain.append('|')
                    domain.append(('move_type','=','out_invoice'))
                ...
                if data['order_view']=='invoice':
                    move_line.sorted('name')
                    for recode in move_line:
                        domain_move_line_analysis=[]
                        domain_move_line_analysis.append(('move_id','in',[recode.id]))
                        if data['analysis_account_id'] and data['analysis_account_id']!=0:
                            analysis_account=self.env['account.analytic.line'].search(
                            ['|',('account_id','in',[data['analysis_account_id']]),('x_plan2_id','in',[data['analysis_account_id']])]
                            )
                            # acc_analysis=self.env['account.analytic.account'].search(
                            #     [('id','in', arg['analysis_account_id'])]
                            # )
                    ...
                    for recode in move_line:
                        ...
                        if data['product_id']:
                            domain_move_line.append(('product_id','in',data['product_id']))
                        if data['product_type']:
                            domain_move_line.append(('product_id.detailed_type','=',data['product_type']))
                        if data['pro_cate_id']:
                            domain_move_line.append(('product_id.categ_id','in',[data['pro_cate_id']]))
                            # name_categ=self.env['product.category'].search(
                            #     [('id','in',[arg['pro_cate_id']])]
                            # )
                            # show_name_categ=name_categ.name
        ...
        # row = 3
        # col = 1
        sheet.merge_range(header_row, header_col, header_row, header_col + 4,'Sale Report',   format_header)
        ...
        if docs[0]["data"]["journal_name"]:
            sheet.write(info_row2, info_col+2, "Journal Name", info_style)
            sheet.merge_range(
                info_row2,
                info_col +3,
                info_row2,
   ...
   # sheet.write(info_row, info_col + 1, '\n'.join(com['name']) , info_style)
        ...
        for obj in docs :
            ...
            sheet.write(row, col+2,  'Partner', header_row_style)
            sheet.write(row, col+3, 'Basic Total', header_row_style)
            sheet.write(row, col+4, 'Total tax items', header_row_style)
            sheet.write(row, col+5,  'Total exempted items', header_row_style)
            sheet.write(row, col+6, 'Total tax', header_row_style)
            sheet.write(row, col+7, 'Total including tax', header_row_style)
            if obj['show_product']:
                sheet.write(row, col+8, 'product', header_row_style)
                sheet.write(row, col+9, 'Quantity', header_row_style)
        # row += 2
        row += 1
        ...
        sheet.write(row, col+3, obj['sum_amount_untaxed'], format_top)
        sheet.write(row, col+4, obj['sum_amount_with_tax'], format_top)
        sheet.write(row, col+5, obj['sum_amount_out_tax'], format_top)
        sheet.write(row, col+6, obj['sum_amount_tax'], format_top)
        sheet.write(row, col+7, obj['sum_amount_total'], format_top)
        if obj["show_product"]:
            sheet.write(row, col+8, '-', format_top)
            sheet.write(row, col+9, obj['sum_quantity'], format_top)
        row += 1
        for item in obj['datas'] :   
            sheet.write(row, col, item['date'], format_date )
            sheet.write(row, col+1, item['name'], format_string)
            sheet.write(row, col+2, item['partner_name'], format_string)
            sheet.write(row, col+3, item['amount_untaxed'], format_num)
            sheet.write(row, col+4,  item['amount_with_tax'], format_num)
            sheet.write(row, col+5,  item['amount_out_tax'], format_num)
            sheet.write(row, col+6,  item['amount_tax'], format_num)
            sheet.write(row, col+7,  item['amount_total'], format_num)
            if obj["show_product"]:
                sheet.write(row, col+8, item['prodect_name'], format_string)
                sheet.write(row, col+9, item['quantity'], format_num)
            row += 1
...
class salespdfReport(models.AbstractModel):
    _name = 'report.yousentech_sale_reports.pdf_sale_temp'
